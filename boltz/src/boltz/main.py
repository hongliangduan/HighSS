...
@rank_zero_only
def download(cache: Path, download_model: bool = True) -> None:
    ...
    # Download the default model only when no custom checkpoint is supplied.
    model = cache / "boltz1_conf.ckpt"
    if download_model and not model.exists():
        click.echo(
...
def predict(
    ...
    # Download the CCD dictionary. Download the default model only when
    # the user did not provide a custom checkpoint.
    download(cache, download_model=checkpoint is None)
